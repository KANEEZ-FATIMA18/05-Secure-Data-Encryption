...
# if 'stored data' not in st.session_state:
#     st.session_state.stored_data = {}

# if 'current page' not in st.session_state:
#     st.session_state.current_page = 'Home'

# if 'last-attempt-time' not in st.session_state:
...
#     #use a passkey to create a consistent key
#     hashed= hashlib.sha256(st.session_state.passkey.encode()).digest()

#     #ensure its valid for Fernet
...
#         #if the passkey is correct, decrypt the data
#             key = generate_key_from_passkey(passkey)
#             cipher = Fernet(key)
...
#     st.write("This system allows you to securely store and retrieve data using encryption.")
...
#     st.info(f"Stored Data Count: {len(st.session_state.stored_data)} encrypted data entries.")
...
#                 else:
#                     st.error("Failed to decrypt data. Incorrect passkey or data ID.")
...
#                 st.warning("Too many failed attempts! Reauthorization required.")
#                 st.session_state.current_page = 'Login'
#                 st.rerun() #updated to rerun the app to show the login page
...
#     #Add simple timeout mechanism

#     if time.time() - st.session_state.last_attempt_time < 10 and st.session_state.failed_attempts > 0:
...
# st.markdown("---")
# st.markdown("Secur Data Encryption System | Educational Project")
